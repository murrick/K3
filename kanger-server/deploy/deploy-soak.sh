...
EXPECTED_CURRENT_UI_TARGET="${KANGER_EXPECTED_CURRENT_UI_TARGET:-/home/murray/sites/kanger-server-0.18-soak-r3-20260807T100726Z}"
...
EXPECTED_CURRENT_JAR_SHA256="${KANGER_EXPECTED_CURRENT_JAR_SHA256:-9a8fb1a0f1505d74fb15343ed0519782abb33b53097d6c0e46fbad7bad962718}"
EXPECTED_CANDIDATE_SERVER="${KANGER_EXPECTED_CANDIDATE_SERVER_VERSION:-server-0.18}"
EXPECTED_SOURCE_HEAD="baaf8f0d0077736666aa9449ff26674f2eec00cd"
EXPECTED_PACKAGING_BRANCH="ops/3.7.0.5-vps-soak"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RECORD_DIR="${KANGER_DEPLOYMENT_RECORD_ROOT:-/root/kanger-deployments}/3.7.0.5-vps-soak-${STAMP}"
CANDIDATE_UI="${SITE_ROOT}/kanger-3.7.0.5-vps-soak-${STAMP}"
...
grep -q '^schema=4$' "${BUNDLE_DIR}/SOURCE.txt"
grep -q '^artifact=3.7.0.5-vps-soak$' "${BUNDLE_DIR}/SOURCE.txt"
...
[[ "${candidate_jar_sha}" != "${current_jar_sha}" ]] || {
  echo "Candidate JAR is byte-identical to current production; refusing ambiguous soak" >&2
  exit 1
}
nginx -t
...
rollback_on_error() {
  ...
  echo "Deployment failed; restoring accepted 3.5.2 Server 0.18 and prior public UI" >&2
  set +e
