...
EXPECTED_SOURCE_HEAD="307042411124ae181e19aea70b50ca7dff6d72a1"
EXPECTED_PACKAGING_BRANCH="ops/3.5.2-server-0.18-vps-soak-r3"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
RECORD_DIR="${KANGER_DEPLOYMENT_RECORD_ROOT:-/root/kanger-deployments}/3.5.2-server-0.18-soak-r3-${STAMP}"
CANDIDATE_UI="${SITE_ROOT}/kanger-server-0.18-soak-r3-${STAMP}"
...
grep -q '^packaging_generation=r3$' "${BUNDLE_DIR}/SOURCE.txt"
grep -q '^server_version=server-0.18$' "${BUNDLE_DIR}/SOURCE.txt"
grep -q '^baseline_insertion_integrated=a70dd388576882aa4cf827a31b3f4724ac339b16$' "${BUNDLE_DIR}/SOURCE.txt"
