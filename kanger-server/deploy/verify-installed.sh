...
echo "${health}" | grep -q '"server_version":"server-0.15"'
echo "${ready}" | grep -q '"server_version":"server-0.15"'
...
echo "KANGER Server 0.15 service, readiness, application/operator loopback confinement and nginx configuration are valid."
