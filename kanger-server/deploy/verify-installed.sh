...
echo "${health}" | grep -q '"server_version":"server-0.17"'
echo "${ready}" | grep -q '"server_version":"server-0.17"'
...
echo "KANGER Server 0.17 service, readiness, application/operator loopback confinement and nginx configuration are valid."
