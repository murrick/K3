...
CANDIDATE_JAR_SHA="$(field candidate_jar_sha256)"
...
[[ -n "${PUBLIC_UI_LINK}" && -n "${PRIOR_PUBLIC_UI_TARGET}" && -n "${CANDIDATE_PUBLIC_UI_TARGET}" ]] || {
  echo "Incomplete deployment record" >&2
  exit 1
}
[[ -n "${PRIOR_JAR_SHA}" && -n "${CANDIDATE_JAR_SHA}" ]] || {
  echo "Incomplete JAR identity in deployment record" >&2
  exit 1
}
[[ -L "${PUBLIC_UI_LINK}" ]] || {
...
[[ -f "${CURRENT_JAR}" ]] || {
  echo "Current JAR is missing: ${CURRENT_JAR}" >&2
  exit 1
}
[[ "$(sha256sum "${CURRENT_JAR}" | awk '{print $1}')" = "${CANDIDATE_JAR_SHA}" ]] || {
  echo "Current JAR no longer matches the recorded soak candidate" >&2
  exit 1
}
[[ -f "${PREVIOUS_JAR}" ]] || {
...
echo "${health}" | grep -q "\"server_version\":\"${CANDIDATE_SERVER_VERSION}\"" || {
  echo "Current server does not report the recorded soak server identity" >&2
  exit 1
}
...
[[ "$(sha256sum "${CURRENT_JAR}" | awk '{print $1}')" = "${PRIOR_JAR_SHA}" ]]
...
restored_state=prior-development-soak-3.7.0.6
full_snapshot_untouched=true
accepted_production_release=release/3.5.2
EOF

echo "DEVELOPMENT_SOAK_ROLLBACK_OK"
echo "server=${PRIOR_SERVER_VERSION}"
echo "jar_sha256=${PRIOR_JAR_SHA}"
...
echo "The full pre-3.7.0.7 state/config snapshot remains available for disaster recovery."
