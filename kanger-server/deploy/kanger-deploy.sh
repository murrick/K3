...
DEFAULT_CHECKOUT_DIR="${XDG_CACHE_HOME:-${HOME}/.cache}/kanger-server-deployer/K3"
...
Fetch, qualify and perform the first KANGER Server application deployment.
The default source is the stable shelf branch ${DEFAULT_REF}.

This script expects the VPS host boundary (Java, systemd, nginx, DNS/TLS and
SSH/sudo access) to be provisioned already. It refuses to replace an existing
KANGER Server installation unless --force is supplied; routine changes belong
to kanger-update.sh.
...
Options:
  ...
  --force                Permit deployment over an existing installation
...
stage_remote_assets() {
  REMOTE_DIR="/tmp/kanger-deploy-${SHORT_COMMIT}-$$"
...
KANGER Server deployment completed.
  operation: ${OPERATION}
...
The deployment receipt is stored at ${REMOTE_RECEIPT_PATH}.
Routine later releases should be installed with kanger-update.sh.
Registration/login continuity should be checked with the production user.
RECEIPT
}
...
log "mode       : $([[ "${FORCE}" == true ]] && printf forced || printf first-install)"
...
log "checking VPS prerequisites and existing installation"
REMOTE_STATE="$(ssh "${SSH_ARGS[@]}" "
  set -e
  ...
  if command -v nginx >/dev/null || test -x /usr/sbin/nginx; then :; else exit 17; fi
  if test -f /etc/systemd/system/kanger-server.service ||
     systemctl is-active --quiet kanger-server.service; then
    printf installed
  else
    printf clean
  fi
...
case "${REMOTE_STATE}" in
  clean)
    OPERATION="first installation"
    ;;
  installed)
    if [[ "${FORCE}" != true ]]; then
      fail "KANGER Server is already installed on ${SSH_TARGET}; use kanger-update.sh or pass --force deliberately"
    fi
    OPERATION="forced redeployment"
    ;;
  *)
    fail "Unexpected VPS preflight result: ${REMOTE_STATE}"
    ;;
esac
