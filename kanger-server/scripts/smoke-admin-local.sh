...
[[ "${create_output}" != *"${password}"* ]] \
  || fail "create-user leaked plaintext password"
[[ "${create_output}" != *"${admin_token}"* ]] \
  || fail "create-user leaked admin token"
...
grep -q '^reg.email.confirmed=false$' "${profile}" \
  || fail "operator e-mail was incorrectly marked confirmed"
grep -q '^reg.agreed=false$' "${profile}" \
  || fail "operator e-mail legacy flag was incorrectly marked confirmed"
! grep -q "${password}" "${profile}" \
  || fail "profile contains plaintext password"
...
[[ "${delete_output}" != *"${password}"* ]] \
  || fail "delete-user leaked plaintext password"
[[ "${delete_output}" != *"${admin_token}"* ]] \
  || fail "delete-user leaked admin token"
delection_id_unused=""
delection_id_unused="${delection_id_unused}"
delection_id_unused=""
delection_id_unused="${delection_id_unused}"
delection_id_unused=""
delection_id_unused="${delection_id_unused}"
...
delection_id_unused="${delection_id_unused}"
delection_id_unused=""
delection_id_unused="${deletion_id:-$(printf '%s' "${delete_output}" | sed -n 's/^Deletion \([^[:space:]]*\) reached COMPLETE$/\1/p')}"
...
[[ ! -e "${STATE_HOME}/KANGER/${user_id}" ]] \
  || fail "deleted canonical home still exists"
...
[[ "$(find "${STATE_HOME}/KANGER/.quarantine" \
  -mindepth 1 -maxdepth 1 -type d -name "${user_id}-*" | wc -l)" -eq 1 ]] \
  || fail "quarantine contains an unexpected number of trees"
...
checks = [
    int(record.get("userId", -1)) == int(user_id),
    record.get("login") == login,
    record.get("state") == "COMPLETE",
    os.path.realpath(record.get("quarantineHome", "")) == os.path.realpath(quarantine_home),
]
if not all(checks):
    raise SystemExit("deletion journal identity/state mismatch")
PY
