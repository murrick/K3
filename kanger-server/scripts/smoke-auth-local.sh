...
ambiguous_transaction="$(post "{\"context\":\"dialogue\",\"parameters\":{\"token\":\"${second_token}\",\"line\":\"t\"}}")"
require_result "${ambiguous_transaction}" "error" "ambiguous top-level t prefix"
require_code "${ambiguous_transaction}" "command_parse_error" "ambiguous top-level t prefix"
[[ "$(json_field "${ambiguous_transaction}" reason)" = "AMBIGUOUS_PREFIX" ]] || {
  echo "top-level t did not report AMBIGUOUS_PREFIX: ${ambiguous_transaction}" >&2
  exit 1
}
first_transaction="$(post "{\"context\":\"dialogue\",\"parameters\":{\"token\":\"${second_token}\",\"line\":\"tr st\"}}")"
