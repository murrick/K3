...
public class Linker {
    ...
    public void link(Right right, boolean logging) {
        ...
        do {
            ...
            while (linkDomains(null, logging)) ;
            while (calcFunctions(null, logging)) ;
...
//                }
    ...
    public boolean calcFunctions(SortedSet<TVariable> tvars, boolean logging) {
        boolean result = false;
        if (tvars == null) {
            tvars = new TreeSet<>();
            tvars.addAll(user.getMind().getDatabase().getTVariables(true));
        ...
        if (tvars.isEmpty()) {
            ...
            if (v != null) {
                ...
                do {
                    user.getMind().getTValues().set(t, v);
                    if (calcFunctions(tvars.headSet(t), logging)) {
                        result = true;
                    }
                } while ((v = t.next(v)) != null);
        ...
        if (level >= master.getPredicate().getRange()) {
            ...
            for (Function f : selected) {
                if (f.isEmpty() && f.isCalculable()) {
                    if (new Calculator(user).calculate(f, logging) > 0) {
                        occurrs = true;
